...
class ZernikeGenerator(object):
    ...
    The user has to provide the pupil input mask (with oversampling, if desired).
    ...
    Parameters
    ----------
        pupil: circular mask, the argument is used as mask
            representing the unit disk.

        pupil_radius: mask radius in pixels
        
        pupil_center: pixel coordinates of the pupil mask center.
                     Defaults to half the pupil size.
    ...
    def __init__(self, pupil, pupil_radius, pupil_center = None):
        self._radius = pupil_radius
        self._shape = pupil.shape
        if pupil_center is None:
            cx,cy = pupil.shape
            pupil_center = (cx/2,cy/2)
        self._center = pupil_center
        self._boolean_mask = pupil.copy()
    ...
    def getZernike(self, index):
        ...
        if index not in list(self._dictCache.keys()):
            res = self._polar(index, self._rhoMap,
                              self._thetaMap)
            self._dictCache[index] = xp.ma.masked_array(
