...
from ekarus.e2e.test_pupil_shift_class import PupilShiftAO
...
def main(tn:str='pupil_shift', subapShift = 0.2):

    print('Initializing devices ...')
    ssao = PupilShiftAO(tn, xp=xp)
    ...
    Rec, _ = ssao.calibrate_modes(ssao.slope_computer, KL, lambdaInM, amps=0.2)
    ...
    subap_masks = 1-(xp.sum(1-ssao.slope_computer._subaperture_masks,axis=0))
    ccd_frame = ssao.ccd.last_frame
    plt.figure()
    myimshow(ccd_frame/xp.max(ccd_frame)-2*subap_masks,title='reference')

    pyr2det = ssao.pupilSizeInPixels*ssao.pyr.oversampling/max(ssao.ccd.detector_shape)
    wedgeAmp = subapShift * pyr2det
    ...
    for k in range(len(phi)):
        ...
        mmWedgeAmp = subapShift/ssao.dm.pixel_scale*pyr2det*1e+3
        sig[k+2,:], _ = ssao.run_loop(lambdaInM, starMagnitude, Rec, m2c, wedgeShift=wedgeShift, save_telemetry_prefix=f'wedge{mmWedgeAmp:1.0f}_ang{phi[k]*180/xp.pi:1.0f}_')
        ccd_frame = ssao.ccd.last_frame
        plt.figure()
    ...
    # Plotting
    masked_input_phases, _, masked_residual_phases, detector_frames, _ = ssao.load_telemetry_data(f'wedge{mmWedgeAmp:1.0f}_ang{phi[0]*180/xp.pi:1.0f}_')
    ...
    showZoomCenter(psf, pixelsPerMAS, shrink=0.8, \
        title = f'Corrected PSF\nStrehl ratio = {xp.exp(-sig[-1,-1]):1.3f}',cmap='inferno') 
    ...
    for k in range(sig.shape[0]):
        ...
        if k == 0:
            ...
            label = f'shift {subapShift/ssao.dm.pixel_scale*pyr2det*1e+3:1.2f} [mm], angle: {phi[k-2]*180/xp.pi:1.0f} [deg]'
