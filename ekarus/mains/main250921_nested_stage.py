...
def main(tn:str='example_nested_stage', 
    ...
    if optimize_gain:
        ...
        print('Finding the best gain for inner loop')
        ...
        print('Finding the best gain for outer loop')
    ...
    if show:
        subap1_masks = xp.sum(cascao.sc1._roi_masks,axis=0)
        plt.figure()
        myimshow(subap1_masks,title='Subaperture masks CCD1')
        subap2_masks = xp.sum(cascao.sc2._roi_masks,axis=0)
        plt.figure()
        ...
        IM1 = xp.asarray(myfits.read_fits(op.join(cascao.savecalibpath,'SC1_IM.fits')))
        _,IM1_eig,_ = xp.asnumpy(xp.linalg.svd(IM1,full_matrices=False))
        IM2 = xp.asaarray(myfits.read_fits(op.join(cascao.savecalibpath,'SC2_IM.fits')))
